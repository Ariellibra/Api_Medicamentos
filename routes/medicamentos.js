...
router.put("/:id", catchAsync(async (req, res) => {
  ...
  const { DROGA, MARCA, PRESENTACION, LABORATORIO, COBERTURA, COPAGO } = req.body;
  const [result] = await db.query(
    `UPDATE medicamentos SET droga=?, marca=?, presentacion=?, laboratorio=?, cobertura=?, copago=? WHERE id=?`,
...
// GET /medicamentos/:id  (dejar SIEMPRE después de las rutas más largas)
